...
public class ObjectGeneration : MonoBehaviour
{
    ...
    /*
     * Generate grass on terrain detail layer
     * TODO : Fix bug - chosen height terrain types doesn't appear to be same as terrain data
     */
    public void GenerateGrass(TileData terrainTile)
    {
        var terrain = terrainTile.terrain;
        var map = terrain.terrainData.GetDetailLayer(0, 0, terrain.terrainData.detailWidth, terrain.terrainData.detailHeight, 0);

        for (int zIndex = 0; zIndex < terrain.terrainData.detailHeight; zIndex++)
        {
            for (int xIndex = 0; xIndex < terrain.terrainData.detailWidth; xIndex++)
            {
                if (terrainTile.chosenHeightTerrainTypes[zIndex, xIndex].name.Equals("ground"))
                {
    ...
    /*
     * Generate rocks as game objects
     */
